...
# En este archivo se agrega ahora las perdidas de energia por friccion.
# Sea agregara la rotacion de las particulas para la siguiente version, asi como la energia cinetica de rotacion de las particulas.
...
function contenedor_circular!(particles::Vector{Particle{N, T}}, radio_Recipiente::T, (; dt, k_n_wall, gamma_n_wall), energia_Mecanica::Vector{T}) where {N, T}
    ...
    for p in particles
        ...
        if delta > 0.0
            ...
            F_n_mag = max(k_n_wall * delta - gamma_n_wall * v_n, 0.0)   # Esta comparacion evita que la fuerza sea negativa, es decir, evita que la fuerza sea atractiva. La fuerza de contacto solo puede ser repulsiva.
            F_n = F_n_mag * n_wall
            ...
            potencia_Disipada_part_pared += gamma_n_wall * v_n^2
        end
    end
...
function contacto_particulas!(particles::Vector{Particle{N, T}}, (; dt, k_n, gamma_n), energia_Mecanica::Vector{T}) where {N, T}
    ...
    for i in 1:num_p
        ...
        for j in (i+1):num_p
            ...
            if d < suma_radios && d > 0.0
                ...
                potencia_Disipada_part_part += gamma_n * v_n_ij^2
            end
        end
    end
...
function simular_sistema()
    ...
    archivo_salida_1 = "resultados/giro_swirling_152.xyz"
    archivo_salida_2 = "datos/datos_swirling_152.csv"
