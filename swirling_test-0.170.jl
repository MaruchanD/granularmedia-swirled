#=
Para las versiones 0.17* se agregan calculos de estabilidad del integrador para verificar que el programa funcione adecuadamente.
=#
using StaticArrays
using LinearAlgebra
...
function contenedor_circular!(particles::Vector{Particle{N, T}}, radio_Recipiente::T, (; dt, k_n_wall, gamma_n_wall, gamma_t_wall, mu_wall), energia_Mecanica::Vector{T}) where {N, T}
    ...
    for p in particles
        ...
        if delta > 0.0
            ...
            v_n_mag = dot(p.v, n_wall)        # Magnitud de la velocidad en la direccion normal a la pared del recipiente
...
function calcular_energia_cinetica_laboratorio!(particles::Vector{Particle{N, T}}, tiempo::T, parametros_Generales::NamedTuple, energia_Mecanica::Vector{T}) where {N, T}
    ...
    for p in particles
        ...
        E_k_t += 0.5 * p.mass * dot(v_abs, v_abs)
        E_k_r += 0.5 * p.inertia * p.omega^2
    end
    
    energia_Mecanica[1] = E_k_t
    energia_Mecanica[6] = E_k_r
end


# -- Funcion para obtener la velocidad de rotacion del cluster de particulas-- #
function calcular_velocidad_rotacion(particles::Vector{Particle{N,T}}) where {N,T}
...
function guardar_datos(archivo::String, tiempo::Float64, energia_Mecanica::Vector{T}) where {T}
    ...
    energia_Cinetica_traslacional = energia_Mecanica[1]
    energia_Cinetica_rotacional = energia_Mecanica[6]
    ...
    energia_Total = energia_Mecanica[1] + energia_Mecanica[6] + energia_Mecanica[2] + energia_Mecanica[4] + energia_Mecanica[3] + energia_Mecanica[5]
    open(archivo, "a") do io
        println(io,"$tiempo,$energia_Cinetica_traslacional,$energia_Cinetica_rotacional,$energia_Potencial_total_part_pared,$energia_Disipada_total_part_pared,$energia_Potencial_total_part_part,$energia_Disipada_total_part_part,$energia_Total")
    end
end
...
function simular_sistema()
    ...
    archivo_salida_1 = "resultados/giro_swirling_170.xyz"
    archivo_salida_2 = "datos/datos_swirling_170.csv"
    ...
    energia_Mecanica = [
        0.0,    # Energia cinetica traslacional
        ...
        0.0     # Energia cinetica rotacional
        ]
