#=
Se esta agregando el calculo del balance energetico entre la fuerza inercial debido a la excitacion orbital, la energia mecanica total y la energia disipada.
Asi como la energia cinetica relativa de las particulas. Se hizo una modificacion mayor a la funcion que exporta los datos al csv.
=#
using StaticArrays
using LinearAlgebra
...
function excitacion_orbital_rampa!(particles::Vector{Particle{N,T}}, tiempo::T, (; amplitud, frecuencia, tau, dt), energia_Mecanica::Vector{T}) where {N, T}                         
    ...
    # Aplicar la aceleracion inercial y calcular la potencia
    for p in particles
        p.a += aceleracion_inercial
        # Producto escalar: F_inercial · v
        potencia_inercial += dot(p.mass * aceleracion_inercial, p.v)
    end
    
    # Acumular el diferencial de trabajo (dW = P * dt)
...
function contenedor_circular!(particles::Vector{Particle{N, T}}, radio_Recipiente::T, (; dt, k_n_wall, gamma_n_wall, gamma_t_wall, mu_wall), energia_Mecanica::Vector{T}) where {N, T}
    ...
    for p in particles
        ...
        if delta > 0.0
            ...
            if v_t_mag > 0.0
                ...
                potencia_Disipada_part_pared += gamma_n_wall * v_n_mag^2 + F_t_mag * v_t_mag
            end
    ...
    energia_Mecanica[5] += potencia_Disipada_part_pared*dt
end
...
function fuerza_total!(particles::Vector{Particle{N, T}}, tiempo::T, parametros_Generales::NamedTuple, energia_Mecanica::Vector{T}, radio_Recipiente::T) where {N, T}
    ...
    excitacion_orbital_rampa!(particles, tiempo, parametros_Generales, energia_Mecanica)
...
function guardar_datos(archivo::String, tiempo::Float64, energia_Mecanica::Vector{T}) where {T}
    ...
    energia_Mecanica_Total = energia_Cinetica + energia_Pot_pared + energia_Pot_part
    ...
    balance = energia_Cinetica_relativa - trabajo_Inercial
    
    open(archivo, "a") do io
        println(io,"$tiempo,$energia_Cinetica,$energia_Pot_pared,$energia_Disip_pared,$energia_Pot_part,$energia_Disip_part,$energia_Mecanica_Total,$trabajo_Inercial,$balance")
    end
end
...
function simular_sistema()
    ...
    sistema = [Particle(@SVector[0.0, 0.0], @SVector[0.0, 0.0], @SVector[0.0, 0.0], 1.0, 0.5, 0.0, 0.0, 0.0, 0.5*masa_Particula*radio_Particula^2)#=, 
    ...
    archivo_salida_1 = "resultados/giro_swirling_171.xyz"
    archivo_salida_2 = "datos/datos_swirling_171.csv"
