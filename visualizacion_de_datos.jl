...
df = CSV.read("datos/datos_swirling_152.csv", DataFrame; header=false)

# 2. Extraer las columnas por posición (o por nombre si tuviera encabezados, ej: df.tiempo)
tiempo            = df[:, 1]
energia_cinetica  = df[:, 2]
energia_potencial_particula_pared = df[:, 3]
energia_disipada_particula_pared = df[:, 4]
energia_potencial_particula_particula = df[:, 5]
energia_disipada_particula_particula = df[:, 6]
energia_total = df[:, 7]
...
limites_x = :auto #(0, 50)         # Define (xmin, xmax) o usa :auto
limites_y = :auto #(-1, 10)      # Define (ymin, ymax) o usa :auto

# 4. Graficar
plot(tiempo, energia_cinetica,
     label="Energía Cinetica",
     xlabel="Tiempo (t)",
     ylabel="Energía (E)",
